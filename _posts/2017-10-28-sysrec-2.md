...
Además presentan el problema de lidiar con nuevos ítems que ingresan al sistema. Digamos que un proveedor de pizza ingresa una nueva pizza, desconocida por todos los usuarios. Es por esta razón que nadie la ha probado ni mucho menos la ha calificado. Cómo hago que esta pizza ingrese en las recomendaciones de los usuarios? (Referirse al *cold start problem* indicado al comienzo)

Además la dimensión de cada ítem tiene relevancia directa con la complejidad de estos algoritmos, debido a algo que se conoce como la *dimensión de la dimensionalidad* (Más en el anexo).
